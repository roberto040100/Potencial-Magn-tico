...
for i in range(n_z):
  ...
  if i>=1 and i<=(n_z-2):
    NUM=mu_0*vp*phi*(Md**2.0)*(T[i+1]-T[i-1])
    DEN=2.0*delta_z*(3.0*k_b*(T[i]**2.0) + mu_0*vp*phi*(Md**2.0)*T[i])

    AUX=NUM/DEN

    C1 = 0.5 - 0.25*AUX*delta_z
    C2 = 0.5 + 0.25*AUX*delta_z
...
for k in range(n_z):
  ...
  if k>=1 and k<=(n_z-2):
    ...
    H[k]= (X3[0][k+1]-X3[0][k-1])/(2*delta_z)
